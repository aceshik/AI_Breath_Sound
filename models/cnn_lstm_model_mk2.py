...
# 1. 데이터 불러오기 및 전처리 (불균형 조정된 버전 사용 + crackle 증강 + wheeze 증강)
...
def load_model(path="models/cnn_lstm_model.pth"):
    """ 저장된 PyTorch 모델 불러오기 """
    model = CNNLSTM()  # 같은 구조의 모델을 먼저 생성
...
# 매번 학습 너무 오래걸려서 threshold 계산은 analyze_thresholds.py


'''
# Threshold 튜닝
best_w, best_c, best_f1 = 0.5, 0.5, 0.0
for w in np.arange(c0.2, 0.61, 0.05):
